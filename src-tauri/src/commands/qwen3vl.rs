...
pub async fn init_qwen3vl_model(
    ...
    // 初始化模型
    info!("开始加载模型文件");
    match state.init_model(final_model_path.clone(), final_tokenizer_path.clone(), model_config) {
        Ok(_) => {
...
pub async fn generate_text(
    ...
    debug!("收到文本推理请求，prompt 长度: {}, max_tokens: {}", request.prompt.len(), max_tokens);
...
pub async fn generate_multimodal(
    ...
    info!("收到多模态推理请求，图像路径: {}, prompt 长度: {}, max_tokens: {}", 
          request.image_path, request.prompt.len(), max_tokens);
...
pub async fn generate_multimodal_from_bytes(
    ...
    let max_tokens = request.max_tokens.unwrap_or(512);
    info!("收到多模态推理请求（字节数据），图像数据大小: {} bytes, prompt 长度: {}, max_tokens: {}", 
          request.image_data.len(), request.prompt.len(), max_tokens);
