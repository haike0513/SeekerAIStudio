...
export type OpenInChatProps = JSX.HTMLAttributes<HTMLDivElement> & {
  text: string;
  providers?: string[];
};

export const OpenInChat: Component<OpenInChatProps> = (props) => {
  ...
  const [, rest] = splitProps(props, ["class", "text", "providers", "children"]);
      ...
      <DropdownMenuContent>
        ...
        <For each={selectedProviders()}>
          {(provider) => (
            <DropdownMenuItem
              onClick={() => {
                const url = provider.createUrl(props.text);
